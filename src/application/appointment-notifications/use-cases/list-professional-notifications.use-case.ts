...
@Injectable()
export class ListProfessionalNotificationsUseCase {
  constructor(
  ...
  async execute(professionalId: string): Promise<NotificationSettingsView> {
    ...
    const tenantId = this.tenantContext.tenantId ?? professional.tenantId;
    return {
      subscriptions: items,
