import { InvalidProfessionalAvatarFileError } from '../exceptions/professional.exceptions';
...
export const PROFESSIONAL_AVATAR_MAX_SIZE_BYTES = 10 * 1024 * 1024;

export const PROFESSIONAL_AVATAR_MAX_SIZE_MB =
  PROFESSIONAL_AVATAR_MAX_SIZE_BYTES / (1024 * 1024);
...
export function assertValidProfessionalAvatarImage(input: {
  mimeType: string;
  body: Uint8Array;
}): void {
  const isAllowedType = PROFESSIONAL_AVATAR_ALLOWED_MIME_TYPES.some(
    (mimeType) => mimeType === input.mimeType,
  );
  const sizeBytes = input.body.length;
  const isAllowedSize =
    sizeBytes > 0 && sizeBytes <= PROFESSIONAL_AVATAR_MAX_SIZE_BYTES;
